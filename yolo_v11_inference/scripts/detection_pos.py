...
session = ort.InferenceSession("yolo11s.onnx", providers=['TensorrtExecutionProvider', 'CUDAExecutionProvider', 'CPUExecutionProvider'])
...
def preprocess(image, input_size=(640, 640)):
    """Preprocess the input image for the ONNX YOLO model."""
    img = cv2.resize(image, input_size)
    img = img.transpose(2, 0, 1)  # Change to channel-first format
    img = img[np.newaxis, :, :, :].astype(np.float32) / 255.0  # Normalize to [0, 1]
    return img
...
def callback(depth_data):
    ...
    try:
        ...
        # Extract results (boxes, scores, and classes) from the model output
        boxes = outputs[0]  # Assuming boxes are in the first output
        scores = outputs[1]  # Assuming scores are in the second output
        classes = outputs[2]  # Assuming class labels are in the third output
        ...
        for box, score, cls in zip(boxes, scores, classes):
            ...
            # Decode the class name (replace with your class names)
            name = "cup" if int(cls) == 0 else "keyboard"  # Adjust indices as needed
