...
public:
    ...
    void spin()
    {
        ...
        for (int i = 0; ros::ok() && i < 150; ++i) {
            ...
            hover_cmd.header.frame_id = "map"; // Use map frame for safety
...
private:
    ...
    void publishSetpoints()
    {
        ...
        cmd.header.frame_id = "map"; // Use map frame
        ...
        cmd.twist.angular.z = cmd.twist.angular.z; // angular smoothing optional
        last_cmd_ = cmd;

        cmd_pub_.publish(cmd);

        ROS_INFO_THROTTLE(2.0, "cmd_vel map: [%.2f, %.2f, %.2f] [%.2f]",
    ...
    geometry_msgs::TwistStamped getFollowCommand()
    {
        ...
        double distance_error = distance - desired_distance_;

        // Map-frame velocities
        double vx = 0.0, vy = 0.0;
        if (distance > 0.01) {
            vx = clamp(dx / distance * distance_error * xy_gain_, -max_linear_speed_, max_linear_speed_);
            vy = clamp(dy / distance * distance_error * xy_gain_, -max_linear_speed_, max_linear_speed_);
        }
