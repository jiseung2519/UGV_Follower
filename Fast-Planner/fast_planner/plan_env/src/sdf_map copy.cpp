...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  node_.param("sdf_map/frame_id", mp_.frame_id_, string("base_link"));
  ...
  vis_points_pub_ = node_.advertise<sensor_msgs::PointCloud2>("/depth/points", 10);
...
void SDFMap::cloudCallback(const sensor_msgs::PointCloud2ConstPtr& img) {
  ...
  // 좌표계 변환: Camera 기준 → ROS 기준 (z-forward → x-forward)
  ...
  // 시각화용 포인트클라우드 퍼블리시 (optional)
  sensor_msgs::PointCloud2 vis_cloud_msg;
  pcl::toROSMsg(latest_cloud, vis_cloud_msg);
  vis_cloud_msg.header.stamp = ros::Time::now();
  vis_cloud_msg.header.frame_id = "base_link";  // RViz fixed frame에 맞게 조정
  vis_points_pub_.publish(vis_cloud_msg);

  // 고정된 로컬 카메라 기준 위치 (odometry 없이)
  Eigen::Vector3d camera_pos = Eigen::Vector3d::Zero();  // (0, 0, 0)

  if (latest_cloud.points.empty()) return;

  this->resetBuffer(camera_pos - mp_.local_update_range_,
                    camera_pos + mp_.local_update_range_);

  pcl::PointXYZ pt;
  Eigen::Vector3d p3d, p3d_inf;
  Eigen::Vector3d max_bound = camera_pos;
  Eigen::Vector3d min_bound = camera_pos;
  ...
  for (size_t i = 0; i < latest_cloud.points.size(); ++i) {
    ...
    if ((devi.cwiseAbs().array() < mp_.local_update_range_.array()).all()) {
      for (int x = -inf_step; x <= inf_step; ++x)
        for (int y = -inf_step; y <= inf_step; ++y)
          for (int z = -inf_step_z; z <= inf_step_z; ++z) {
            p3d_inf = p3d + Eigen::Vector3d(x, y, z) * mp_.resolution_;

            min_bound = min_bound.cwiseMin(p3d_inf);
            max_bound = max_bound.cwiseMax(p3d_inf);
  ...
  // 로컬 바운드 업데이트
  max_bound[2] = std::max(max_bound[2], mp_.ground_height_);
  posToIndex(max_bound, md_.local_bound_max_);
  posToIndex(min_bound, md_.local_bound_min_);
  boundIndex(md_.local_bound_min_);
  boundIndex(md_.local_bound_max_);
