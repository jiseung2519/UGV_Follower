...
void SDFMap::cloudCallback(const sensor_msgs::PointCloud2ConstPtr& img) {
  ...
  //TODO change coordinate transformation : Image coordinate --> ROS(robot) coordinate
  ...
  Eigen::Quaterniond q_world_base;   // base_link → world 방향의 orientation
  Eigen::Vector3d t_world_base;      // base_link의 위치 (world 좌표계 기준)

  // base_link의 pose (global 기준)
  t_world_base = md_.camera_pos_;    // base_link의 위치
  q_world_base = md_.camera_q_;      // base_link의 orientation (w, x, y, z)

  for (auto& pt : latest_cloud.points) {
    Eigen::Vector3d p_base(pt.x, pt.y, pt.z);               // local 좌표 (base_link 기준)
    Eigen::Vector3d p_global = q_world_base * p_base + t_world_base;  // 변환 적용

    pt.x = p_global.x();    // 전역 좌표계로 업데이트
    pt.y = p_global.y();
    pt.z = p_global.z();
  }
  ...
  vis_cloud_msg.header.frame_id = "odom";  // RViz fixed frame에 맞게 조정
