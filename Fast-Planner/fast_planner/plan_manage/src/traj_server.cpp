...
ros::Publisher cmd_vis_pub, pos_cmd_pub, traj_pub, traj_path_pub, pos_mavros_pos_cmd_pub;

nav_msgs::Odometry odom;

quadrotor_msgs::PositionCommand cmd;
geometry_msgs::PoseStamped mavros_cmd;
// double pos_gain[3] = {5.7, 5.7, 6.2};
// double vel_gain[3] = {3.4, 3.4, 4.0};
double pos_gain[3] = { 1.7, 1.7, 6.2 };
double vel_gain[3] = { 2.4, 2.4, 4.0 };
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  mavros_cmd.header.stamp = time_now;
  mavros_cmd.header.frame_id = "odom";

  mavros_cmd.pose.position.x = pos(0);
  mavros_cmd.pose.position.y = pos(1);
  mavros_cmd.pose.position.z = pos(2);

  tf::Quaternion q = tf::createQuaternionFromYaw(yaw);

  mavros_cmd.pose.orientation.x = q.x();
  mavros_cmd.pose.orientation.y = q.y();
  mavros_cmd.pose.orientation.z = q.z();
  mavros_cmd.pose.orientation.w = q.w();

  // pos_mavros_pos_cmd_pub.publish(mavros_cmd);
...
int main(int argc, char** argv) {
  ...
  ros::Subscriber odom_sub = node.subscribe("/mavros/local_position/odom", 50, odomCallbck);

  cmd_vis_pub = node.advertise<visualization_msgs::Marker>("planning/position_cmd_vis", 10);
  pos_cmd_pub = node.advertise<quadrotor_msgs::PositionCommand>("/position_cmd", 50);
  pos_mavros_pos_cmd_pub = node.advertise<geometry_msgs::PoseStamped>("/mavros/setpoint_position/local", 50);
  traj_pub = node.advertise<visualization_msgs::Marker>("planning/travel_traj", 10);
  // traj_path_pub = node.advertise<nav_msgs::Path>("planning/travel_traj/path", 10);
