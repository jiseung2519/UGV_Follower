...
void KinoReplanFSM::init(ros::NodeHandle& nh) {
  ...
  bspline_path_pub_ = nh.advertise<nav_msgs::Path>("/planning/bspline_path", 10);
}

void KinoReplanFSM::publishPathFromBspline(const plan_manage::Bspline& bspline_msg) {
  nav_msgs::Path path_msg;
  path_msg.header.stamp = ros::Time::now();
  path_msg.header.frame_id = "odom";

  size_t N = bspline_msg.pos_pts.size();
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    // Position
    pose_stamped.pose.position = bspline_msg.pos_pts[i];

    // Orientation from yaw
    if (i < bspline_msg.yaw_pts.size()) {
      double yaw = bspline_msg.yaw_pts[i];
      tf::Quaternion q = tf::createQuaternionFromYaw(yaw);
      tf::quaternionTFToMsg(q, pose_stamped.pose.orientation);
    } else {
      // fallback: no orientation info
  ...
  bspline_path_pub_.publish(path_msg);
}
...
bool KinoReplanFSM::callKinodynamicReplan() {
  ...
  if (plan_success) {
    ...
    publishPathFromBspline(bspline);
